---
title: "Ch.7 — Coordinator & Concurrency — 協調與並行"
...
**問題所在：** LLM 本質上是一個無狀態的函式：接受輸入 tokens，輸出 tokens，然後結束。它無法「等待」非同步操作、無法在回應中途暫停、也無法自行觀察前幾輪的工具執行結果。工具執行是真正的非同步 I/O：一個 `BashTool("npm test")` 可能需要 30 秒；一個 `WebFetch` 可能觸發網路請求。這兩個現實——LLM 的無狀態性與工具執行的非同步性——需要一個協調層來橋接。
