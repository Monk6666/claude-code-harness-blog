...
傳統 subagent 的 prompt 是「背景說明（briefing）」；fork 的 prompt 是「接下來做什麼（directive）」——兩種完全相反的寫作策略。源碼的說明：
...
**`isPDFSupported()` 的 conditional injection：** PDF 支援說明段落本身通過 runtime 函數動態插入——表示 Claude Code 的多模態能力因部署環境而異。讀到「This tool can read PDF files」這段文字的 Claude，是在支援 PDF 的環境中運行的；沒讀到這段的 Claude，不應該嘗試讀取 PDF。Prompt 作為「能力宣告」，而非靜態說明書。
