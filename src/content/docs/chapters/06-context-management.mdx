...
**問題**：`getSystemContext` 和 `getUserContext` 都是用 `lodash-es/memoize` 快取的函數，在 session 期間各自獨立。但如果只清除其中一個，下次組裝 system prompt 時，stale 的那個快取可能產生不一致的假設——舊的 user context（CLAUDE.md 內容）和新的 system context（含新注入）或反之，形成概念上的不一致。
