...
在進入細節之前，先看清楚整座山的形狀。以下是全書 13 章的結構，每一章都在前一章的基礎上增加複雜度。
...
| Ch.08 | Skills & Plugin System — 技能與插件 | Ch.02 | `initBundledSkills`、MCP server 協議、Plugin 安裝流程 |
...
| **Plugins/MCP 插件** | 連接 MCP server、發現 MCP 工具並注入工具清單、管理插件生命週期 | `src/services/mcp/client.ts`（`getMcpToolsCommandsAndResources`）、`src/utils/plugins/` | Ch.08 |
...
在 `src/main.tsx` 的模組頂層，三個關鍵函數按嚴格順序呼叫，這些呼叫發生在所有其他 import 之前：
...
`src/entrypoints/init.ts` 中的 `init()` 函數處理無需使用者信任即可執行的初始化：
...
`src/setup.ts` 中的 `setup()` 函數處理與工作目錄和 session 環境相關的初始化：

```typescript
// src/setup.ts — setup() 函數的關鍵步驟順序
...
你的輸入抵達 `src/query.ts` 的 `query()` 函數。`query()` 呼叫 `queryLoop()`，這是整個執行引擎的核心 generator 函數。`queryLoop` 收到的 `params` 包含：`systemPrompt`（已組裝好的提示詞）、`messages`（對話歷史）、`toolUseContext`（工具集合）、以及 `userContext`。

此時，`queryLoop` 的第一個任務是**組裝第一個 API 呼叫**，但在送出之前，它需要確保 context 已準備好。

### 2. Context 組裝：CLAUDE.md 注入 + System Prompt 建構（→ Ch.06, Ch.11）

在 API 呼叫發出前，`src/context.ts` 的 `getSystemContext()` 和 `getUserContext()` 已在啟動時被 prefetch（`startDeferredPrefetches()` 中觸發）。這兩個函數使用 `memoize` 快取結果。
